...
Après avoir testé [ElasticSearch](http://dev.david.pilato.fr/?p=126), me voici parti pour regarder ce monde étrange qu'on appelle le [NoSQL](http://fr.wikipedia.org/wiki/Nosql)...
...
Alors, nous décidons de lancer un noeud ElasticSearch sur une machine et d'y ajouter le [plugin river CouchDB](http://www.elasticsearch.org/guide/reference/river/couchdb.html). Puis nous activons ce plugin pour ouvrir une rivière (river) vers la première instance de CouchDB... Et là, encore un miracle... Nos documents se déversent sans aucun effort dans ElasticSearch.
